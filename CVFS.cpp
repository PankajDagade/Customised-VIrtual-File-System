...
void man(char *name)
{
	...
	else if(strcmp(name,"lseek") == 0)
	{
		...
		printf("Usage :lseek File_Name ChangeInOffset StartPoimt\n");
	}
...
void DisplayHelp()
{
	...
	printf("write : To Write the content from files\n");
...
void ls_file()
{
	...
	printf("\nFile Name\tlnode number\tFilesize\tLink count\n");
	printf("---------------------------------------------------------------\n");
	while(temp != NULL)
	{
		if(temp->FileType != 0)
...
int fsat_file(int fd)
{
	...
	printf("\n ------Satistical Information about file----------------\n");
	printf("File name %s\n",temp->FileName);
	printf("lnode Number %d\n",temp->InodeNumber);
	printf("File size : %d\n",temp->FileSize);
...
int main()
{
	...
	while(1)
	{
		...
		if(count == 1)
		{
			...
			else if(strcmp(command[0],"exist") == 0)
			{
		...
		else if(count == 3)
		{
			...
			else if(strcmp(command[0],"open") == 0)
			{
				...
				if(ret == -1)
					printf("ERROR : Incorrect parameter\n");
				if(ret == -2)
					printf("ERROR : File is not presen\n");
				if(ret == -3)
					printf("ERROR :Permission dined\n");
				continue;
			}
